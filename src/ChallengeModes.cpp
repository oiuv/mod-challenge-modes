...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "开启硬核模式（一条命，不可复活）", 0, SETTING_HARDCORE);
        }
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "开启半硬核模式（死亡失去装备和金币）", 0, SETTING_SEMI_HARDCORE);
        }
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "开启自制装备模式（仅可使用玩家制造装备）", 0, SETTING_SELF_CRAFTED);
        }
        if (sChallengeModes->challengeEnabled(SETTING_ITEM_QUALITY_LEVEL) && !playerSettingEnabled(player, SETTING_ITEM_QUALITY_LEVEL))
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "开启普通装备模式（仅可使用普通品质装备）", 0, SETTING_ITEM_QUALITY_LEVEL);
        }
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "开启慢速经验模式（0.5倍经验）", 0, SETTING_SLOW_XP_GAIN);
        }
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "开启龟速经验模式（0.25倍经验）", 0, SETTING_VERY_SLOW_XP_GAIN);
        }
        if (sChallengeModes->challengeEnabled(SETTING_QUEST_XP_ONLY) && !playerSettingEnabled(player, SETTING_QUEST_XP_ONLY))
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "开启仅任务经验模式", 0, SETTING_QUEST_XP_ONLY);
        }
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "开启铁人模式（地狱级限制）", 0, SETTING_IRON_MAN);
        }
